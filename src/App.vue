...
export default {
  ...
  methods: {
    ...
    updateDoneStatus(itemId) {
      const needToBeUpdated = this.ToDoItems.find(item => item.id === itemId)
      needToBeUpdated.done = !needToBeUpdated.done
    },
    ...
    deleteToDo(toDoId){
      const deletedItem = this.ToDoItems.findIndex(item => item.id === toDoId/*.id*/) // 写成 toDoId.id 功能必错
      this.ToDoItems.splice(deletedItem, 1)
    }
  },
  computed: {
    listSummary() {
